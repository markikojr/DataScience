...
def predict():
    ...
    if request.method == 'POST': 
       ...
       tweets = tweepy.Cursor(api.search, q=search_term, lang="en", since=start_date, until=end_date).items(number_of_tweets)
    ...
    #RENDERING .HTML
    return render_template('results.html', search = search, start_date = start_date, end_date = end_date, mean_score = mean_score ,num_of_tweets = num_of_tweets, tweet_dict = user_dict, number_of_positives = number_of_positives, number_of_negatives = number_of_negatives, number_of_neutrals = number_of_neutrals)
